...
<display:table name="survivalClasses" id="row" pagesize="5" class="displaytag" requestURI="survivalClass/manager/list.do">

	<security:authorize access="hasRole('MANAGER')">
		<display:column>
			<jstl:if test="${row.manager.userAccount.username == pageContext.request.userPrincipal.name && row.trip.startDateTrip > date}">
