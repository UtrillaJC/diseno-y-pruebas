...
<form:form action="trip/manager/edit.do" modelAttribute="trip">
	...
	<form:input path="ticker" disabled="true"/>
	...
	<b><form:label path="hasValues"><spring:message code="trip.tags"/>:&nbsp;</form:label></b>
	<form:textarea path="hasValues"/>
	<div id="tagsExplanation">
		<spring:message code="trip.eachTag"/>
		<br/>
		<spring:message code="trip.availableTags"/>:
		<jstl:forEach var="tag" items="${tags}">
			<jstl:out value="- ${tag.name}"/>
			<br/>
		</jstl:forEach>
	</div>
	<form:errors path="hasValues" cssClass="error"/>
	<br/>
	...
	<jstl:if test="${trip.id == 0 || trip.publicationDate > date}">
