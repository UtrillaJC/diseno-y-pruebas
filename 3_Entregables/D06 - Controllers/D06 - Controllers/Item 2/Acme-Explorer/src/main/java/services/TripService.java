...
public class TripService {
	...
	public Trip save(final Trip trip) {
		...
		Assert.notNull(trip);
		Assert.isTrue(trip.getStartDateTrip().before(trip.getEndDateTrip()));
		Assert.isTrue(trip.getPublicationDate().before(trip.getStartDateTrip()));
		...
		} else if (this.checkPrincipalAdmin(trip))
			res = this.tripRepository.save(trip);
		return res;
	}
	...
	public void deleteByManager(final Manager manager) {
		...
		for (final Trip t : trips) {
			...
			this.hasValueService.deleteByTrip(t);
		}
