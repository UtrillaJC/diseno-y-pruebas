...
<display:table name="rangers" id="row" pagesize="5" class="displaytag" requestURI="admin/listSuspicious.do">
	...
	<display:column>
		<jstl:if test="${row.suspicious }">
					<a href="admin/desactivate.do?rangerId=${row.id}"><spring:message code="admin.desactivate"/></a>
		</jstl:if>
		
		<jstl:if test="${!row.suspicious }">
					<a href="admin/activate.do?rangerId=${row.id}"><spring:message code="admin.activate"/></a>
		</jstl:if>
	</display:column>		
	<spring:message var="suspicious" code="admin.ranger.suspicious"/>
	<display:column property="suspicious" title="${suspicious}"/>
